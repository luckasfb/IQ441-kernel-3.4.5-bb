...
static u8 gup_load_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *data, u8 *path)
{
    ...
    for (i = 0; i < mask_num; i++)
    {
	    ...
	    TPD_DWN_DEBUG("ic_version is 0x%4x .\n",ic_version);
            //The third condition
	     //fw_version=86  ic_version
	   //add jone at 2012 /9/18
            if ((fw_version > ic_version||(update_msg.ic_fw_msg.version&0xFF)==0xBA||(update_msg.ic_fw_msg.version&0xFF)==0xBB)				
...
u8 gt818x_downloader(struct i2c_client *client)
{
    u8 flag = 0;
